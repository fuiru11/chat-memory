# Chat Memory

Personal conversation review & reflection tool for Claude Code sessions.

## Architecture

- **Single-page app** (`index.html`) — vanilla JS, no framework, no build step
- **Python sync service** (`sync.py`) — parses Claude Code JSONL logs → structured JSON, serves the viewer
- **Skills** — `/cm-nap` (save session), `/cm-sleep` (daily journal), `/cm-backfill` (historical sessions), `/cm-fix` (diagnose & repair)
- **LaunchAgent** — `com.chatmemory.server.plist` (sync daemon), `com.chatmemory.journal.plist` (daily journal fallback at 05:00)

## Data Flow

```
Claude Code JSONL → sync.py (+ auto-migration via data/version.json)
                        ↓
                   data/conversations/*.json + data/index.json
                        ↓
/cm-nap skill → data/summaries/*.md + data/highlights.json + data/artifacts.json
           → data/segments.json + data/topics.json (conversation segments & topic threads)
                        ↓
/cm-sleep skill → data/journal/{date}.md
/cm-backfill skill → same outputs as /cm-nap, for historical sessions
/cm-fix → diagnose + repair + migrate
```

## Key Files

| File | Purpose |
|------|---------|
| `index.html` | Main viewer SPA (Home, Conversations, Journal tabs) |
| `sync.py` | JSONL parser, data sync, schema migration, web server |
| `config.json` | Local config (port, persona, paths) |
| `data/` | All conversation data (gitignored) |
| `marked.min.js` | Markdown renderer (vendored) |

## Viewer Tabs (max 3, progressive disclosure)

1. **Home** — Weekly insight, active topics, recent highlights/artifacts/conversations
2. **Conversations** — Sidebar list + chat view, calendar filter, topic filter, search (searches segment text)
3. **Journal** — Daily reflection entries (4 sections: Daily Reflection, What I Learned, Understanding the User, Growth Notes)

## Design Principles

- **Token-conscious** — minimize AI token usage, prefer local processing
- **Sustainable** — features should be low-friction enough to stick
- **Three-layer data** — segments (searchable fragments) + summaries (session overview) + full conversations (detail), linked by topics
- **No framework** — vanilla HTML/CSS/JS, single file, no build
- **Privacy first** — conversation data is gitignored, no PII in public outputs

## Code Style

- Compact but readable JS (no unnecessary whitespace, but clear variable names)
- CSS custom properties for theming
- SVG icons inline (no emoji for UI elements)
- Chinese UI labels where appropriate, English for section headers

## Segments & Topics

- **Segment** = a chunk of consecutive messages about the same topic within a session
- **Topic** = a thread linking related segments across sessions (auto-generated by /cm-nap)
- Segments replace tags as the primary retrieval mechanism
- `data/segments.json` — flat array of segment objects (id, sessionId, topic, summary, startTs, endTs, topicId)
- `data/topics.json` — map of topicId → {name, description, segments[], lastUpdated}
- Search covers segment topics + summaries (not just session title/one_line)
- Tags kept in summaries for backward compatibility but no longer primary

## Interaction Patterns

- Highlight card click → jump to source conversation + scroll to message
- Highlight/Artifact "View all" → modal overlay (not new tab, keep 3-tab limit)
- Artifact click → preview modal (render md, highlight code, show images)
- Segment card click → scroll to that part of the conversation
- Topic pill/name click → topic thread modal (timeline of related segments across sessions)
- Topic thread item click → jump to that session + segment position
- Journal entries rendered from `data/journal/{date}.md` markdown files
